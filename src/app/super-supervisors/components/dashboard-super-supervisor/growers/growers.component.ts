...
export class GrowersComponent implements OnInit {
	...
	tableUserDataFilter = [];
	bugsData = [];
	constructor(private fb: FormBuilder, private districtService: DistrictService, private cus: CreateUserService, private gs: GrowerService, private cs: ContractService, private ps: ProductsService, private fs: FieldsService, private is: InvitesService, private rs: ReportsService, private bugService: BugService, private toastr: ToastrService) { }
	...
	filterGrowers() {
		...
		for (let i = 0; i < this.tableUserData.length; ++i) {
			for (let j = 0; j < this.growerTable.length; ++j) {
				if (this.tableUserData[i]['user_id'] == this.growerTable[j]['supervisor_id']) {
					this.filterGrowerData.push(this.growerTable[j]);
				}
			}
		}
	...
	createNewGrowers() {
		...
		this.gs.addGrower(this.add_growersForm.value).subscribe((data) => {
			...
			console.log(error);
			this.toastr.error('Create Grower!', 'Error!');

		}, () => {
			// this.add_growersForm.reset();
			this.clearAddGrowerForm();
			this.toastr.success('Create Grower!', 'Completes!');
		});
	}
	...
	submitEditData() {
		...
		this.gs.editGrower(this.edit_growersForm.value).subscribe((data) => {
			...
			console.log(error)
			this.toastr.error('Edit Grower!', 'Error!');

		}, () => {
			// this.edit_growersForm.reset();
			this.clearEditGrowerForm();
			this.toastr.success('Edit Grower!', 'Completes!');
	...
	afterEdit(data) {
		console.log(data);
		let obj = this.growerTable.find(x => x['_id'] === data['_id']);
		let index = this.growerTable.indexOf(obj);
		this.growerTable[index] = data;
	}
	...
	editDataFromList(data, index) {
		...
		// this.selectedCountry = this.findSupervisorById(data.supervisor_id)
	...
	displayFieldsTable(id) {
		...
		console.log('???', this.displayFieldData);
	...
	clearAddGrowerForm() {
		...
		this.add_growersForm.patchValue({
			first_name: "",
			last_name: "",
			cell_phone: "",
			email: "",
